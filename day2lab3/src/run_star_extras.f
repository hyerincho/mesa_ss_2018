...
! ***********************************************************************
      ...
      subroutine my_mlt(  &
         ...
         real(dp), intent(inout) :: mlt_basics(:) 
         real(dp), intent(inout), pointer :: mlt_partials1(:)
         integer, intent(out) :: ierr

         real(dp) ::  delta, B, P_gas, gamma, del, new_grada
         ierr = 0
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         ...
         if (k>0) then
            ...
            if (B == 0) then
               ...
               gamma = Cp/Cv
               delta = B**2 / (B**2 + 4*pi*gamma*P_gas)
            end if
            del = min(delta,0.1)
         else
            del = 0d0
         ...
         call mlt_eval(  &
            cgrav, m, r, T, rho, L, P, &
            chiRho, chiT, Cp, Cv, csound, X, opacity, new_grada,  &
            gradr_factor, gradL_composition_term, &
            alpha_semiconvection, semiconvection_option, &
            thermohaline_coeff, thermohaline_option, &
            dominant_iso_for_thermohaline, &
