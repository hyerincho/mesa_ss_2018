...
!     include 'standard_run_star_extras.inc'
   ...
   if (Tearth > s% x_ctrl(1)) then
      if (ABS(Tearth - s% x_ctrl(1)) > 1e-6) then
      s% dt = s% dt/2
      ...
      integer function how_many_extra_profile_columns(id, id_extra)
         ...
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         how_many_extra_profile_columns = 1 !0
...
!         return
         ...
         do k = 1, s% nz
            s% extra_heat(k) = 0.1*Lsun/(0.05*Msun)*exp(-1*s% m(k)/(0.05*Msun))
